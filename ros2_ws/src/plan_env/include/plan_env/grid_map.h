...
#include <plan_env/fused_observation_evidence.h>
...
struct MappingParameters {
  ...
  double observation_timeout_sec_;
  double max_cloud_pose_skew_sec_;
  int sensor_sync_queue_size_;
  int diagnostic_max_samples_;
  int diagnostic_history_depth_;
  string observation_diagnostics_topic_;
...
struct OccupancyTransitionProvenance {
  Eigen::Vector3i global_voxel_index{Eigen::Vector3i::Zero()};
  Eigen::Vector3d hit_endpoint_world{Eigen::Vector3d::Zero()};
...
struct MappingData {
  ...
  bool has_ray_pose_, has_cloud_, has_fused_observation_;
  // 首帧有效传感器输入前，循环栅格仍是全空初值，可以直接把窗口中心
  // 放到机器人附近；任何占据写入都会先锁定该中心，之后只允许正常滑窗清理。
  bool sliding_center_initialized_;
  bool last_cloud_was_canonical_empty_;
  int64_t last_ray_pose_stamp_ns_;
  int64_t last_cloud_stamp_ns_;
  int64_t pending_cloud_stamp_ns_;
  int64_t last_fused_stamp_ns_;

  // depth image projected point cloud

  vector<Eigen::Vector3d> proj_points_;
  // 1 表示障碍命中端点，0 表示有测量证据的自由端点。
  vector<char> proj_points_endpoint_hit_;
  int proj_points_cnt;

  // flag buffers for speeding up raycasting

  vector<short> count_hit_, count_hit_and_miss_;
  // 每个缓存体素中来自显式 free ray 的 miss 次数，用于因果审计。
  vector<short> count_explicit_free_miss_;
  // 当前连续占据 epoch 的 free→occupied 阈值穿越来源。只有真正穿越
  // occupancy 门限的 hit 观测会写入，直到该体素变回 free 或被 reset。
  vector<std::uint64_t> occupancy_transition_hit_observation_sequence_;
  // 只保存当前仍 occupied 的阈值穿越 epoch；key 是当前 ring address，
  // value 同时带 global index，滑窗复用前必须清除。
  std::unordered_map<int, OccupancyTransitionProvenance>
      occupancy_transition_provenance_by_address_;
...
public:
  ...
  inline int getRotationSweepOccupancy(Eigen::Vector3d pos);
  ...
  // 返回最近已接纳的传感器观测时间戳；即使尚待融合也保留该只读基线。
  int64_t observationStampNs() const;
  // 返回真正完成 raycast/fusion 的非空观测代数；规范空云不推进该代数。
  std::uint64_t fusedObservationSequence() const noexcept;
  // 证明 baseline 之后至少有 required 个采集于 yaw 稳定后的不同非空观测。
  FusedObservationEvidence fusedObservationEvidenceAfter(
      std::int64_t settle_stamp_ns,
      std::uint64_t baseline_sequence,
      std::int64_t now_ns,
      std::uint64_t required_observations = 3U) const noexcept;
...
private:
  ...
  int setCacheOccupancy(
      Eigen::Vector3d pos, int occ, bool explicit_free_ray = false);
  Eigen::Vector3d closetPointInMap(const Eigen::Vector3d& pt, const Eigen::Vector3d& ray_pos);
  std::uint32_t updateSlidingMap(const Eigen::Vector3d& center);
  ...
  void publishObservationDiagnostics();
  ...
  shared_ptr<message_filters::Subscriber<sensor_msgs::msg::PointCloud2>>
      cloud_sub_;
  ...
  rclcpp::Publisher<
      scan_planner_msgs::msg::GridMapObservationDiagnostics>::SharedPtr
  ...
  bool pending_observation_diagnostics_valid_{false};
  std::uint64_t observation_diagnostics_sequence_{0};
  std::atomic<std::uint64_t> fused_observation_sequence_{0U};
  mutable std::mutex fused_observation_history_mutex_;
  std::deque<FusedObservationRecord> fused_observation_history_;
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos, double yaw) {
  ...
  // 行驶轨迹按 SCAN 社区版的双圆柱语义检查：圆柱中心沿当前轨迹切向
  // 分别位于 base_link 前后。不能把所有行驶点都扩大为“任意 yaw 原地旋转”
  // 外接圆，否则会把实际可沿长轴通过的窄走廊误判为不可达。
  const Eigen::Vector3d heading(std::cos(yaw), std::sin(yaw), 0.0);
  const Eigen::Vector3d front =
      pos + mp_.double_cylinder_offset_ * heading;
  const Eigen::Vector3d rear =
      pos - mp_.double_cylinder_offset_ * heading;

  const int front_occupancy =
      getInflateOccupancyFromBuffer(front, md_.occupancy_buffer_inflate_);
  if (front_occupancy != 0) return front_occupancy;

  return getInflateOccupancyFromBuffer(
      rear, md_.occupancy_buffer_inflate_);
}

inline int GridMap::getRotationSweepOccupancy(Eigen::Vector3d pos) {
  if (!pos.allFinite() || !isInMap(pos)) return -1;

  // 只有确实要原地转向时才查询任意 yaw 的扫掠外接圆。半个 voxel
  // 对角线覆盖连续 offset 圆盘到离散 cell 之间的缝隙。
  const double sweep_radius = std::abs(mp_.double_cylinder_offset_);
  const double cell_cover = std::sqrt(0.5) * mp_.resolution_;
  const double query_radius = sweep_radius + cell_cover;
  const int sweep_steps =
      static_cast<int>(std::ceil(query_radius / mp_.resolution_));
  for (int x = -sweep_steps; x <= sweep_steps; ++x) {
    for (int y = -sweep_steps; y <= sweep_steps; ++y) {
      const Eigen::Vector2d offset_xy(
          x * mp_.resolution_, y * mp_.resolution_);
      if (offset_xy.norm() > query_radius) continue;
      const Eigen::Vector3d sample =
          pos + Eigen::Vector3d(offset_xy.x(), offset_xy.y(), 0.0);
      const int occupancy =
          getInflateOccupancyFromBuffer(sample, md_.occupancy_buffer_inflate_);
