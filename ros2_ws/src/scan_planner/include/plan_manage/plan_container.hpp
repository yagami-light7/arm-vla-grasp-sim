...
namespace scan_planner
{
  ...
  struct PlanParameters
  {
    ...
    // reference 初始速度剖面使用的加速度占物理上限比例；最终发布门仍使用 max_acc_。
    double reference_profile_acceleration_scale_;
    // 空旷折线仅在碰撞、顺序走廊和耗时三重门通过时采用平滑候选。
    bool reference_free_guide_refine_enabled_;
    double reference_free_guide_refine_minimum_duration_gain_;
    double planning_horizon_;
    // 参考路径局部轨迹只允许数值量级的回退，防止前视航向翻转。
    double max_reference_reverse_distance_;
    double max_reference_reverse_speed_;
    double reference_corridor_max_deviation_;
    double reference_obstacle_corridor_max_deviation_;
    double reference_corridor_max_progress_lead_;
    double reference_obstacle_corridor_max_progress_lead_;
    double reference_corridor_initial_progress_tolerance_;
    double reference_corridor_progress_measurement_tolerance_;
  ...
  struct LocalTrajData
  {
    ...
    bool reference_corridor_checked_{false};
    bool reference_corridor_safe_{false};
    double maximum_trajectory_deviation_{0.0};
    double maximum_guide_anchor_deviation_{0.0};
    double maximum_guide_progress_lead_{0.0};
    double reference_corridor_deviation_limit_{0.0};
    double reference_corridor_progress_lead_limit_{0.0};
    std::vector<Eigen::Vector3d> ordered_reference_guide_;
  };
