...
namespace scan_planner
{
  ...
  bool UniformBspline::checkFeasibility(double &ratio, bool show)
  {
    ...
    if (!std::isfinite(limit_vel_) || limit_vel_ <= 0.0 ||
        !std::isfinite(limit_acc_) || limit_acc_ <= 0.0 ||
        !std::isfinite(feasibility_tolerance_) ||
        feasibility_tolerance_ < 0.0 || p_ < 1 ||
        control_points_.size() == 0 || control_points_.cols() <= p_ ||
        !control_points_.allFinite() ||
        !std::isfinite(interval_) || interval_ <= 0.0 ||
        u_.size() != m_ + 1 || !u_.allFinite())
      return false;
    for (Eigen::Index index = 1; index < u_.size(); ++index)
      if (!(u_(index) > u_(index - 1)))
        return false;
    const double duration = u_(m_ - p_) - u_(p_);
    if (!std::isfinite(duration) || duration <= 0.0)
      return false;

    bool fea = true;

    const Eigen::MatrixXd &P = control_points_;

    /* check vel feasibility and insert points */
    double max_vel = 0.0;
    const double enlarged_vel_lim =
        limit_vel_ * (1.0 + feasibility_tolerance_) + 1e-4;
    if (!std::isfinite(enlarged_vel_lim))
      return false;
    for (int i = 0; i < P.cols() - 1; ++i)
    {
      const Eigen::VectorXd vel =
          p_ * (P.col(i + 1) - P.col(i)) /
          (u_(i + p_ + 1) - u_(i + 1));
    ...
    /* acc feasibility */
    double max_acc = 0.0;
    const double enlarged_acc_lim =
        limit_acc_ * (1.0 + feasibility_tolerance_) + 1e-4;
    if (!std::isfinite(enlarged_acc_lim))
      return false;
    ...
    ratio = max(
        max_vel / limit_vel_, sqrt(max_acc / limit_acc_));
  ...
  void UniformBspline::scaleTimeUniformly(const double &ratio)
  {
    if (!std::isfinite(ratio) || ratio <= 0.0)
      throw std::invalid_argument("B-spline 时间缩放比例必须是有限正数");

    const Eigen::VectorXd scaled_knots = u_ * ratio;
    const double scaled_interval = interval_ * ratio;
    if (!scaled_knots.allFinite() || !std::isfinite(scaled_interval) ||
        scaled_interval <= 0.0)
      throw std::overflow_error("B-spline 时间缩放结果溢出");

    // 构造器保证有效时间域从 u_p=0 开始。全部 knot 同比缩放后，
    // de Boor 的归一化权重不变，空间曲线保持一致；速度和加速度分别
    // 按 1/ratio 与 1/ratio^2 缩放。
    u_ = scaled_knots;
    interval_ = scaled_interval;
  }
