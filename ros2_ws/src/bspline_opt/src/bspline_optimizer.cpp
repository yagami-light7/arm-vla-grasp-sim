...
// 本文件基于 SCAN-Planner ROS 2 community 分支；此处以标准容器替代非标准变长数组。
#include <algorithm>
#include <chrono>
#include <cmath>
#include <limits>
...
namespace scan_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    // reference/path controller 会执行整条局部 B-spline；末 1/3 同样必须参与
    // 障碍分段，否则最终安全门只会确定性拒绝而 rebound 永远看不到障碍。
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        if (j < 0) // fail to get the obs free point
        {
          ...
          rebound_failure_state_.observe(
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (ret == ASTAR_RET::SUCCESS)
        {
          ...
          if (path.size() < 2)
          {
            ...
            rebound_failure_state_.observe(
                classifyCollisionSegmentAstarPath(path.size()));
  ...
  {
    rebound_failure_state_.reset();
    setBsplineInterval(ts);

    bool flag_success = rebound_optimize();
    rebound_failure_state_.complete(flag_success);
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
      ...
      memcpy(q.data(), cps_.points.data() + 3 * start_id, q.size() * sizeof(q[0]));
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        for (double t = tm; t <= tmp + 1.0e-9; t += t_step)
        {
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    bool flag_safe = true;
    bool solver_result_usable = true;
    int iter_count = 0;
    do
    {
      lbfgs::lbfgs_parameter_t lbfgs_params;
      ...
      else if (result == lbfgs::LBFGSERR_ROUNDING_ERROR &&
               std::isfinite(final_cost) && cps_.points.allFinite())
      {
        // L-BFGS 在近直线、已接近极小值的轨迹上可能找不到更小步长。
        // 此时只保留有限的当前迭代点；调用方仍必须重新通过碰撞、
        // ordered-reference 走廊和完整动力学门，不能把该返回码视作收敛。
        RCLCPP_WARN(
            rclcpp::get_logger("bspline_opt"),
            "Refine line search stopped by rounding limits; keep the finite "
            "candidate for downstream safety validation");
      }
      else
      {
    ...
    return flag_safe && solver_result_usable;
  }
